...
public class KMeans {
    ...
    private int[] _withLabel; // if original labels exist, load them to
    // _withLabel
    // by comparing _label and _withLabel, we can
    // compute accuracy.
    // However, the accuracy function is not defined
    // yet.
    ...
    public double[][] clustering(double[][] data, int numClusters, int niter, double[][] centroids) {
        _data = data;
        _nrows = _data.length;
        _ndims = _data[0].length;
        ...
        double[][] _best_centroids = _centroids;
        // System.out.println("ok");
        double[][] c1 = _centroids;
        double threshold = 0.001;
        int round = 0;
        int nstart = 3;
        double minWithinss = Double.MAX_VALUE;
        ...
        do {
            do {
                ...
                c1 = updateCentroids();
                round++;
            } while (!((niter > 0 && round >= niter) || converge(_centroids, c1, threshold)));
            double withinss = sumSquaresError();
            if (withinss < minWithinss) {
                minWithinss = withinss;
                _best_centroids = _centroids;
            }
        } while (nstart-- > 0);

        // System.out.println("Clustering converges at round " + round);
        return _best_centroids;
    }
    ...
    public double sumSquaresError() {
        ...
        for (int i = 0; i < _data.length; ++i) {
            ...
            for (int j = 0; j < _data[i].length; ++j) {
                result += Math.pow(_data[i][j] - _centroids[c][j], 2);
            }
        }
        return result;
    }
    ...
    private double[][] updateCentroids() {
        ...
        for (int i = 0; i < _nrows; i++) {
            ...
            for (int j = 0; j < _ndims; j++) {
                newc[cn][j] += _data[i][j]; // update that centroid by adding
